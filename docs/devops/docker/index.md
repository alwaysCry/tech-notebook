...
- Registry：一个集中存储、分发镜像的服务，可包含多个仓库
  ...
  仓库通常以`[Registry 用户名]/[软件名]`的形式命名，可包含同一软件的不同版本（标签）镜像，后者可以`<仓库名>:<标签>`形式具体指定，无标签则视为 latest<!-- _以 Ubuntu 镜像为例：ubuntu 为仓库名，`ubuntu:16.04`或`ubuntu:18.04`为具体版本镜像；若忽略标签，则视为 `ubuntu:latest`_ -->
